...
from egg.zoo.color_signaling.archs import Sender, Receiver, ContinousGame
...
def dump(game, test_data, device, gs):
    ...
    with torch.no_grad():
        ...
        receiver_outputs = game.receiver(messages.softmax(dim=1), receiver_inp)
...
def main(params):
    ...
    dump(game, val_loader, device, gs=True)
    core.close()
